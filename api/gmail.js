// ============================================================
// /api/gmail.js — Proxy sécurisé Gmail pour WorkMate
// Lecture Gmail progressive par lots avec pagination.
...
async function readMessagesInBatches(messageIds, accessToken, batchSize = 20) {
  const emails = [];
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Lecture par page pour éviter les timeouts Vercel.
    // Le navigateur appelle cette route plusieurs fois si l'utilisateur choisit "tout".
    const pageSize = clampNumber(req.body?.pageSize || req.body?.maxEmails, 1, 100, 100);
    const pageToken = cleanText(req.body?.pageToken || "");
    const query = cleanText(req.body?.query || "");
    const includeSpamTrash = Boolean(req.body?.includeSpamTrash);
    ...
    const emails = await readMessagesInBatches(ids, accessToken, 20);
