...
function loadingHTML(text = "WorkMate AI analyse...") {
...
async function loadHistory(limitCount = 50, targetId = "history-list") {
  ...
  try {
    ...
    target.innerHTML = `<div class="history-item">Erreur de chargement : ${escapeHTML(error.message)}</div>`;
  }
}
...
function renderAssistantMessage(role, text) {
  ...
  div.innerHTML = `
    <small>${role === "user" ? "Vous" : "WorkMate AI"}</small>
...
window.sendAssistantMessage = async function () {
  ...
  try {
    ...
    await saveAnalysis("assistant", `Chat IA — ${text.slice(0, 45)}...`, "Réponse WorkMate AI", answer);
  } catch (error) {
...
window.loadGmailEmails = async function () {
  ...
  try {
    ...
    const result = await callWorkMateAI(`
Analyse ces ${data.emails.length} emails Gmail et structure ta réponse EXACTEMENT ainsi :
...
window.switchView = function (name) {
  ...
  safeSetText("topbar-title", viewTitles[name] || "WorkMate AI");
