...
function showGmailConnected(connected) {
  const ui = document.getElementById("gmail-connect-ui");
  const ok = document.getElementById("gmail-connected");
  if (ui) ui.style.display = connected ? "none" : "block";
  if (ok) ok.style.display = connected ? "flex" : "none";
}

window.saveGmailToken = function () {
  const input = document.getElementById("gmail-token-input");
...
window.loadGmailEmails = async function() {
  ...
  setBtnBusy("btn-gmail", true, "📬 Analyse en cours...");
  try {
...
window.switchEmailMode = function(mode) {
  ...
  if (mode === "gmail") {
    ...
    showGmailConnected(!!(token || gmailAccessToken));
  }
};
