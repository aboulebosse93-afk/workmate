...
function titleFromText(text = "") {
  let clean = String(text || "")
    .replace(/Pièces jointes\s*:.*/gis, "")
    .replace(/^(Explique simplement ce sujet\s*:|Résume ce contenu et donne-moi les actions importantes\s*:|Travaille dans le contexte du projet .*?:)/i, "")
    .replace(/[#*_`>\[\]()]/g, " ")
    .replace(/\s+/g, " ")
    .trim();
  ...
  if (!clean) {
    const now = new Date();
    return `Discussion du ${now.toLocaleDateString("fr-FR")} ${now.toLocaleTimeString("fr-FR", { hour: "2-digit", minute: "2-digit" })}`;
  }
...
function smartConversationTitle(conversation) {
  if (!conversation) return "Discussion";

  const current = String(conversation.title || "").trim();
  if (current && current !== "Nouvelle discussion") return current;

  const firstUser = (conversation.messages || []).find((m) => m.role === "user" && String(m.content || "").trim());
  if (firstUser) return titleFromText(firstUser.content);

  const date = conversation.createdAt ? new Date(conversation.createdAt) : new Date();
  return `Discussion du ${date.toLocaleDateString("fr-FR")} ${date.toLocaleTimeString("fr-FR", { hour: "2-digit", minute: "2-digit" })}`;
}

function normalizeConversationTitles() {
  const conversations = getConversations();
  let changed = false;

  const fixed = conversations.map((conversation) => {
    if (conversation.title && conversation.title !== "Nouvelle discussion") return conversation;

    const title = smartConversationTitle(conversation);
  ...
  if (changed) {
    localStorage.setItem(STORAGE_KEYS.conversations, JSON.stringify(fixed));
  }
...
function renderConversationsList() {
  ...
  let conversations = normalizeConversationTitles();
  ...
  target.innerHTML = conversations.map((c) => `
    <div class="conversation-row ${c.id === currentId ? "active" : ""}">
      <button class="conversation-link" onclick="loadConversation('${escapeHTML(c.id)}')">
        <span>${escapeHTML(smartConversationTitle(c))}</span>
      </button>
...
function renderDashboardConversations() {
  ...
  target.innerHTML = conversations.map((c) => `
    <button class="mini-row" onclick="loadConversation('${escapeHTML(c.id)}'); switchView('assistant')">
      <strong>${escapeHTML(smartConversationTitle(c))}</strong>
...
window.deleteConversation = function (id) {
  ...
  const ok = confirm(`Supprimer la discussion "${smartConversationTitle(conversation)}" ?`);
  if (!ok) return;
...
window.exportCurrentConversation = function () {
  ...
  const content = [
    `# ${smartConversationTitle(conversation)}`,
    "",
    ...(conversation.messages || []).map((m) => `## ${m.role === "user" ? "Vous" : "Assistant"}\n\n${m.content}`)
  ].join("\n\n");

  downloadTextFile(`${smartConversationTitle(conversation)}.md`, content);
...
function saveMessageToCurrentConversation(role, content) {
  ...
  updateConversation(id, (conversation) => {
    ...
    const shouldAutoTitle =
      role === "user" &&
      (!currentTitle ||
        currentTitle === "Nouvelle discussion" ||
        /^Discussion du \d{2}\/\d{2}\/\d{4}/.test(currentTitle));

    const title = shouldAutoTitle ? titleFromText(content) : conversation.title;
...
// v36 — Réunion propre + titres de discussions améliorés
window.clearReunionFields = function () {
  safeSetValue("reunion-agenda", "");
  safeSetValue("reunion-input", "");
  safeSetText("reunion-chars", "0");
  ...
  showToast("Nouvelle réunion prête.");
  setTimeout(() => $("reunion-input")?.focus(), 80);
};

window.clearTranscript = function () {
  safeSetValue("reunion-transcript", "");
  fullTranscript = "";
  showToast("Transcription effacée.");
  setTimeout(() => $("reunion-transcript")?.focus(), 80);
};

window.renameUntitledConversations = function () {
  normalizeConversationTitles();
  renderConversationsList();
  renderDashboardConversations();
  showToast("Noms des discussions mis à jour.");
};
