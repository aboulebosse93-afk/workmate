...
function renderProjects() {
  ...
  target.innerHTML = projects.map((project) => `
    <article class="project-card">
      ...
      <div>
        <button class="btn btn-soft" onclick="setAssistantPrompt('Travaille dans le contexte du projet ${escapeHTML(project.name)} : ')">Utiliser</button>
...
window.runAgent = async function () {
  ...
  box?.classList.add("visible");
  if (body) body.innerHTML = loadingHTML("Agent en réflexion...");
  setBtnBusy("btn-agent", true, "Agent...");

  try {
    const context = `
Conversations récentes : ${getConversations().slice(0, 5).map((c) => c.title).join(", ")}
Actions ouvertes : ${getLocalActions().filter((a) => !a.done).slice(0, 20).map((a) => a.text).join(" | ")}
Documents : ${getLocalDocuments().slice(0, 10).map((d) => d.title).join(", ")}
Projets : ${(getProjects ? getProjects() : []).map((p) => p.name).join(", ")}
`;

    const plan = await callWorkMateAI(`
Tu es en Mode Agent. Prépare un plan multi-étapes pour atteindre l'objectif.
Ne fais aucune action sensible sans demander confirmation.
Structure : objectif, contexte, étapes, actions à confirmer, résultat attendu.
...
${context}
    ...
    extractActionsFromText(plan, "Agent");
    addLog("agent", "Plan agent généré");
  } catch (error) {
    if (body) body.textContent = "Erreur : " + error.message;
  } finally {
...
window.addEventListener("appinstalled", () => {
  ...
  addLog?.("pwa", "Application WorkMate installée");
});

window.installWorkMateApp = async function () {
  if (!deferredInstallPrompt) {
    alert("L’installation n’est pas disponible pour le moment. Essaie d’ouvrir le site en HTTPS, puis recharge la page.");
    return;
  }
  ...
  if (choice?.outcome === "accepted") {
    addLog?.("pwa", "Installation acceptée");
  }
};
